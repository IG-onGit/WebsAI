...
# Existing code base to enhance

This is the existing code base that you must enhance based on user request:
...
0. "db.sql" - enhanced separated SQL code block if needed (skippable) - ready to execute in database;
2. "page.html" - enhanced separated HTML code block if needed (skippable) - responsive structure without inline styles and scripts;
3. "style.css" - enhanced separated CSS code block if needed (skippable) - responsive styles;
4. "script.js" - enhanced separated jQuery JavaScript code block if needed (skippable) - handling complex actions;
5. "code.php" - enhanced separated PHP code block (mandatory, even if no methods, class is required) - handling business logic;
...
- Each code block should contain complete enhanced functional code;
...
You must enhance required existing structured code blocks for this user request:
[[message]]
